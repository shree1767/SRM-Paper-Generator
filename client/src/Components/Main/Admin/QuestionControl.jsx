...
const QuestionControl = () => {
  ...
  const { user, cookies } = useContext(UserContext);
    ...
    const fetchData = async () => {
      try {
        ...
        const responseSub = await fetch("http://localhost:8000/question/subjective",{
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${cookies.jwt}`,
  ...
  const deleteObjective = async (_id) => {
    try {
      ...
      const responseSub = await fetch(`http://localhost:8000/question/subjective/${_id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${cookies.jwt}`,
        },
      ...
      if(response.status === 201 || responseSub.status === 201){
  ...
  return (
    <div className="mx-auto flex flex-col space-y-5  p-8 md:px-20 mt-20 h-full w-screen justify-center bg-[#F6F6F6]">
      <div className="text-3xl font-semibold mx-auto mt-10 mb-5">All Questions</div>
